import {
  ...
  hiddenClientsOn, noteFor, clientNotes,
} from './schedule'
...
// ══════════════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════════════
// The same day, worked out once per request rather than three times
//
// Vercel paused the platform mid-shift — not for requests (487K against an
// allowance of a million) but for CPU: twelve hours against four. Measured,
// a tick costs 51ms of CPU, and this function is 6.7ms of it. Three of the
// six handlers a tick runs call it with the SAME inputs, so a fifth of every
// tick was spent working out one identical answer three times.
//
// Safe to reuse because the answer is deterministic — shuffle-check pins
// that: fifty runs on one input give one result — and because nothing
// mutates the plan it is handed. Both are worth keeping true; if a caller
// ever starts writing to a plan, this memo is what will make that a bug in
// somebody ELSE's screen.
//
// Matched on identity for the big things and on content for the small ones.
// The row arrays come from readSheetCached, which hands back the same array
// object for the life of its TTL, so identity is both exact and free. The
// week-off set and the leave map are rebuilt per request, so those are
// compared by a cheap signature instead.
const MEMO_MAX = 4

function memoSignature(a) {
  const names = a.weekOffNames ? [...a.weekOffNames].sort().join(',') : ''
  const leave = a.leaveMap ? Object.keys(a.leaveMap).sort().join(',') : ''
  const over  = a.overridesMap ? Object.keys(a.overridesMap).sort().join(',') : ''
  // Hidden clients and hour notes reach the plan as MODULE STATE rather than
  // as arguments, so they are invisible to an identity check on the rows. A
  // plan memoised a moment before an admin hides a client would go on being
  // served with that client still on somebody's board — the memo quietly
  // undoing the very thing that was just asked for. Both are a handful of
  // entries, so folding them in costs nothing and cannot go stale.
  const hid  = [...hiddenClientsOn(a.date)].sort().join(',')
  const note = JSON.stringify(clientNotes())
  return `${a.date}|${a.today}|${a.nowHour}|${names}|${leave}|${over}|${hid}|${note}`
}

function memoGet(args) {
  const g = globalThis.__cautioPlanMemo || (globalThis.__cautioPlanMemo = [])
  const sig = memoSignature(args)
  for (const e of g) {
    if (e.sig !== sig) continue
    if (e.shiftRows !== args.shiftRows) continue
    if (e.updateRows !== args.updateRows) continue
    if (e.breakRows !== args.breakRows) continue
    if (e.vehicleMap !== args.vehicleMap) continue
    return e.plan
  }
  return null
}

function memoPut(args, plan) {
  const g = globalThis.__cautioPlanMemo || (globalThis.__cautioPlanMemo = [])
  g.unshift({
    sig: memoSignature(args), plan,
    shiftRows: args.shiftRows, updateRows: args.updateRows,
    breakRows: args.breakRows, vehicleMap: args.vehicleMap,
  })
  if (g.length > MEMO_MAX) g.length = MEMO_MAX
}

export function computeDayPlan(args) {
  const hit = memoGet(args)
  if (hit) return hit
  const plan = computeDayPlanUncached(args)
  memoPut(args, plan)
  return plan
}

function computeDayPlanUncached({
  ...
  // ── Clients an admin has taken off THIS day ─────────────────────────
  //
  // Resolved once, here, because this is the one place that knows which day is
  // being planned. Everything below is handed the same set, so a client that
  // is off for the day is off it on the employee's board, on Hour by hour and
  // in every total — rather than being hidden on one screen and counted as
  // missed on another.
  ...
  const dueAt = (hour) => {
    ...
    // A client taken off the day is not work that was missed — it is work
    // that was never due. Leaving it in the denominator would mark the floor
    // down for not doing something nobody asked them to do.
    hidden.forEach(c => s.delete(c))
    return [...s]
  }
  ...
  // And the same fact asked about the CLIENT rather than the client-hour:
  // has anybody filled this at any point today? That is a different question
  // from "was this slot done", and it is the one "Still not updated" asks —
  // a client filled at eleven has been filled, even if its eight o'clock slot
  // is still outstanding and shows as pending on the board.
  //
  // It has to include the hour still in progress. Filling a client at 11:15
  // is an update made between seven this morning and now, so it takes the
  // client off the list immediately — waiting for the hour to finish would
  // leave somebody looking at a client they had just done.
  const filledToday = new Map()     // client -> the time it was last recorded
  rowsForDate.forEach(r => {
    if (!(r[5] || '').toString().trim()) return
    doneAt.set(`${r[4]}|${r[3]}`, r[1] || '')
    const client = (r[3] || '').toString().trim()
    if (client) filledToday.set(client, r[1] || '')
  })
  ...
  for (const hour of businessHourOrder()) {
    ...
    const owners = {}
    const dist = distributeClientsForHour(hour, pool, vehicleMap, {}, true, hidden)
    Object.entries(dist).forEach(([name, cs]) => {
      if (cs.length) owners[name] = cs.map(c => ({
        ...
        // The standing note for this client at this hour, if there is one.
        // It travels with the client so every screen that shows the client
        // can show the note, rather than each one looking it up its own way.
        note: c.note || noteFor(c.client, hour) || null,
    ...
    if (unassigned.length > 0) {
      // `hidden` here too, or the audit measures the hour against a list the
      // split was never given: a client the admin took off the day is reported
      // as one nobody is covering. Seen with an empty pool — all three clients
      // came back as unassigned including the hidden one, which is the Command
      // Center's loudest alarm firing for work that was deliberately cancelled.
      const audit = auditHourAssignment(hour, pool, vehicleMap, {}, true, hidden)
  ...
  return {
    ...
    hours,
    // client -> the time it was last filled today, by anybody, at any hour
    // including the one still running. See where it is built.
    filledToday,
    byEmployee,
...
export function staleClientsFrom(plan, nowHour) {
  const ORDER = businessHourOrder()
  const seen = new Map()
  const filled = plan.filledToday || new Map()
  ...
  plan.hours.forEach(h => {
    ...
    Object.entries(h.owners).forEach(([name, cs]) => {
      cs.forEach(c => {
        ...
        // The day-wide answer, which is the one this list is asking for. A
        // client filled at eleven is filled, even though its eight o'clock
        // slot is still outstanding — and the hour in progress counts, so
        // filling something at 11:15 clears it now rather than at noon.
        //
        // Without this the admin and the employee disagreed: the employee's
        // panel already tested the whole day (see updatedToday on the board),
        // so a client filled in the running hour vanished from their list and
        // stayed on the admin's.
        const at = filled.get(c.client)
        if (at !== undefined) { rec.filledToday = true; if (at) rec.lastUpdatedAt = at }
  ...
  return [...seen.values()]
    // Untouched all day. One update anywhere, by anyone, at any hour from
    // seven this morning until right now — and it is not on this list.
    .filter(r => r.done === 0 && !r.filledToday && r.pending > 0)
    .map(r => {
