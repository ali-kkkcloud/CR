...
import { isSupersededBreak } from './attendance'
...
export async function summariseDay(date, { shiftRows, updateRows, breakRows, weekOffNames }) {
  ...
  ;(breakRows || []).slice(1).forEach(r => {
    if (r[2] !== date) return
    // Rows the repair superseded carry zero minutes anyway, but a day put
    // away for good should not be summarised from history rows at all.
