...
export function updateChip(args) {
  const s = statusOf(args)
  // "Updated" stays on the front even in the short form. Without it a bare
  // "03:30 pm · Nesiya" beside a client is just a time — it never says what
  // happened at it, and the whole point of the line is that this is the LAST
  // time anybody touched this client.
  if (s.tone === 'done')      return { text: stamp(shortClock(s.at)), tone: s.tone }
  if (s.tone === 'elsewhere') {
    const text = [stamp(shortClock(s.at)), s.by].filter(Boolean).join(' · ')
...
const stamp = (when) => (when ? `Updated ${when}` : 'Updated')
