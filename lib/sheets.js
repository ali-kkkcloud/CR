...
export function getAwayOnBreakNames(breakRows, dates = [], minMinutes = 20) {
  ...
  ;(breakRows || []).slice(1).forEach(r => {
    ...
    if (!name || !dateSet.has(r[2])) return
    if ((r[6] || '').toString().trim() !== 'Active') return
    // The row's date is the SHIFT date, so a break opened before midnight on a
    // night shift has to be resolved against both candidate dates — the same
    // rule every other time in these sheets follows.
    let startedAt = null
    for (const d of dateSet) {
      const p = parseISTDateTime(d, r[3])
      if (!p) continue
      const t = p.getTime()
