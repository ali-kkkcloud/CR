  ...
  try {
    ...
    await Promise.all(ranges.map(async range => {
      try { deliver(range, await readSheet(spreadsheetId, range, { breaker: true })) }
        ...
        // A range whose TAB does not exist is the one failure that is not an
        // error: it is an answer, and the answer is "nothing". Remembered so
        // it stops taking the rest of the batch down with it.
