...
export function getOnShiftNamesFromLog(shiftLogRows, dates = []) {
  const latestStatus = {}
  ...
  shiftLogRows.slice(1).forEach(r => {
    ...
    latestStatus[name] = (r[6] || '').toString().trim()
  })
  return new Set(
    Object.entries(latestStatus).filter(([, st]) => st === 'Active').map(([name]) => name)
  )
}
