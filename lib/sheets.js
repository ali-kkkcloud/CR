...
export async function fetchClientVehicleCounts() {
  const now = Date.now()
  if (_vehicleCache && (now - _vehicleCacheTime) < VEHICLE_CACHE_TTL) {
  ...
  try {
    for (const tab of ['Infants', 'Others']) {
      let rows = []
      try {
        rows = await readSheet(SOURCE_SHEET_ID, `${tab}!A:B`)
      } catch (e) {
        console.error(`fetchClientVehicleCounts: could not read tab ${tab}`, e.message)
        continue
      }
    ...
    console.error('fetchClientVehicleCounts error:', err)
  }
  return map
}

export function getVehicleCount(vehicleMap, clientName) {
