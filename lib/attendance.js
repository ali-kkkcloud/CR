import {
  ...
  todayStr, yesterdayStr, nowIST, parseISTDateTime, parseOperatingDateTime, findOpenShiftRow, getShiftOverridesForDate, getWatchlistNames, TTL,
} from './sheets'
import { getEmployeeShift } from './schedule'
import { clampClaim, heartbeatTrustMinutes } from './integrity'
...
//
// These used to be one number, and that was the flaw an extension was found
// exploiting: it forced the browser's claim to "just now" on every poll, the
// claim was the newest thing here, and so the break never fired.
//
// WORK is what the server watched happen — the shift starting, a client
// actually filled. A browser cannot move it without doing the job.
//
// CLAIMED is what the browser says about its own user. It is worth having:
// somebody watching feeds through a quiet hour is present with nothing to
// record. But it is a claim, and lib/integrity.js decides how far a claim
// is believed past the work that supports it.
//
// A resume counts as presence rather than work. Coming back from a break is
// a real thing a person does, and the idle clock should start again — but it
// is one click, so it cannot buy a fresh window of unrecorded time.
export function activityClocks(emp, ctx) {
  const { shiftRows, updateRows, breakRows, dates, nowMs } = ctx
  const empId = (emp.empId || '').toString().trim()

  let workAt = null, claimedAt = null
  const newer = (a, b) => (b != null && (a == null || b > a) ? b : a)

  const open = findOpenShiftRow(shiftRows, empId, dates)
  if (open) {
    // The shift's own start is verified: the employee clocked in.
    workAt    = newer(workAt,    resolveMoment(open.row[3], dates, nowMs))
    claimedAt = newer(claimedAt, resolveMoment(open.row[HEARTBEAT_COL - 1], dates, nowMs))
  }
  ...
  if (ctx.heartbeatOverride && ctx.heartbeatOverride[empId] != null) {
    claimedAt = newer(claimedAt, ctx.heartbeatOverride[empId])
  }
  ...
  updateRows.slice(1).forEach(r => {
    ...
    workAt = newer(workAt, resolveMoment(r[1], dates, nowMs))
  })

  // A resume is verified: the server watched the employee end their own
  // break. It has to reset the idle clock outright — anything less and
  // somebody who presses Resume is put straight back on a break by the next
  // sweep, which is a loop they cannot get out of.
  breakRows.slice(1).forEach(r => {
    if ((r[0] || '').toString().trim() !== empId) return
    if (!dates.includes(r[2])) return
    if (r[4]) workAt = newer(workAt, resolveMoment(r[4], dates, nowMs))
  })

  return { workAt, claimedAt }
}

// How far past their last recorded work this employee's browser is believed.
//
//   0     not at all — the person is on the watchlist, so their idle time is
//         measured from what they have actually recorded. Faked mouse
//         movement buys nothing, because nothing is listening to it.
//   null  entirely, as the platform has always done.
//
// See getWatchlistNames in lib/sheets.js for why this is a list of names a
// person chose rather than a rule applied to the whole floor.
function trustFor(emp, ctx) {
  const name = (emp?.name || '').toString().trim().toLowerCase()
  if (name && ctx?.watchlist?.has(name)) return 0
  return heartbeatTrustMinutes()
}

export function lastActivityAt(emp, ctx) {
  const { workAt, claimedAt } = activityClocks(emp, ctx)
  return clampClaim(workAt, claimedAt, { trustMinutes: trustFor(emp, ctx) }).at
}
...
export async function sweepAutoBreaks(employees, heartbeatOverride = null) {
  ...
  const [shiftRows, updateRows, breakRows, overridesToday, overridesYesterday, watchlist] = await Promise.all([
    readSheetCached(CRM_SHEET_ID, `${TABS.SHIFT_LOG}!A:H`, TTL.LIVE),
    readSheetCached(CRM_SHEET_ID, `${TABS.CRM_UPDATES}!A:L`, TTL.LIVE),
    readSheetCached(CRM_SHEET_ID, `${TABS.BREAKS}!A:H`, TTL.LIVE),
    getShiftOverridesForDate(dates[0]),
    getShiftOverridesForDate(dates[1]),
    // Cached on the roster's long TTL and tolerant of a missing tab, so this
    // costs nothing on the usual path and watches nobody until the tab exists.
    getWatchlistNames(),
  ])
  ...
  const overridesMap = { ...overridesYesterday, ...overridesToday }
  const ctx = { shiftRows, updateRows, breakRows, dates, nowMs, heartbeatOverride, overridesMap, watchlist }
