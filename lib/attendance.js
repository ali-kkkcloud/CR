import {
  ...
  todayStr, yesterdayStr, nowIST, parseISTDateTime, findOpenShiftRow, getShiftOverridesForDate,
  calcDuration,
} from './sheets'
...
// A date, in the sheets' own DD/MM/YYYY, for a moment in time.
function ddmmyyyyOf(ms) {
  const d = new Date(ms)
  return `${String(d.getDate()).padStart(2,'0')}/${String(d.getMonth()+1).padStart(2,'0')}/${d.getFullYear()}`
}
...
export async function sweepShiftAutoClose(employees) {
  ...
  for (const emp of employees) {
    ...
    for (let i = shiftRows.length - 1; i >= 1; i--) {
      ...
      const endDate = ddmmyyyyOf(endMs)
      const duration = calcDuration(r[2], r[3], endDate, endTime)
