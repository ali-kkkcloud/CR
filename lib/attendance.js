...
// The moment this shift is due to finish.
//
// A break must not outlive the shift it belongs to. Somebody who goes quiet
// at ten to six and never comes back should read as "on break until six",
// not as still sitting on a break the next morning — so the sweep needs to
// know where the shift ends even when nobody clicked End Shift.
//
// The adjusted window wins where there is one: arriving after the half hour
// adds an hour to the end, and taking OT pushes it further, both of which are
// recorded in Shift_Overrides. Otherwise the roster's own end hour applies.
export function shiftEndMoment(empName, shiftDate, startTimeStr, overridesMap) {
  ...
  const startAt = parseISTDateTime(shiftDate, startTimeStr)
  if (!startAt) return null

  const end = new Date(startAt)
  end.setHours(endHour, 0, 0, 0)
  // A night shift finishes on the far side of midnight, so an end hour that
  // lands at or before the start belongs to the next day.
  if (end <= startAt) end.setDate(end.getDate() + 1)
  return end.getTime()
}
...
export function evaluateAutoBreak(emp, ctx, today) {
  ...
  const endMs = shiftEndMoment(emp.name, shift.date, shift.row[3], ctx.overridesMap)
