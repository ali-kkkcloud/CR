...
// ALL_EMPLOYEES start/end for this scheduling check only.
...
export function getEmployeeShift(empName) {
  return ALL_EMPLOYEES.find(e => e.name === empName) || null
}

// Early Start rounding rule:
// - Round the click time to the nearest hour (< 30 min -> floor, >= 30 min -> ceil).
// - If that rounded hour is EARLIER than the employee's normal scheduled
//   start, they're eligible to pull their whole shift forward by exactly
//   that many hours (start AND end both shift back by the same amount —
//   duration is preserved). No cap — arriving 3 hours early pulls 3 hours.
// - If the rounded hour is at-or-after their normal start, nothing changes
//   (that's just a normal / late start, handled elsewhere).
// Returns null when no early-start adjustment applies.
export function computeEarlyStart(emp, now) {
  const h = now.getHours()
  const m = now.getMinutes()
  const roundedHour = (m < 30 ? h : h + 1) % 24

  // How many hours "early" is roundedHour relative to emp.start, accounting
  // for the possibility of a night-shift employee (start could be a large
  // hour like 22 while roundedHour wraps past midnight to a small number).
  let hoursEarly
  if (emp.end <= emp.start) {
    // Night shift: "early" means roundedHour is behind emp.start on the
    // clock but within the pre-shift window (not the other side of the
    // wrap). Compute as: distance going backward from emp.start to roundedHour.
    hoursEarly = (emp.start - roundedHour + 24) % 24
    // Guard against it wrapping the "wrong way" (i.e. arriving so early it
    // looks like it's actually very late) — cap the sensible early window
    // at 12 hours; beyond that treat as no adjustment.
    if (hoursEarly > 12) hoursEarly = 0
  } else {
    hoursEarly = emp.start - roundedHour
  }

  if (hoursEarly <= 0) return null // not early enough (or arrived late) — no change

  const newStart = ((emp.start - hoursEarly) % 24 + 24) % 24
  const newEnd   = ((emp.end   - hoursEarly) % 24 + 24) % 24
  return { start: newStart, end: newEnd, hoursShifted: hoursEarly }
}

// Late Start rule (symmetric inverse of Early Start):
// - Employees get a full grace hour — logging in ANY time during
//   [scheduledStart, scheduledStart+1) counts as normal, no adjustment.
// - If they haven't started AT ALL by the top of scheduledStart+1, that
//   grace hour is simply lost (their clients for it go unfilled — NOT
//   redistributed, since it was their grace window). From scheduledStart+1
//   onward, they're marked "Week Off" until they actually log in.
// - Whenever they DO log in late, their whole shift shifts FORWARD by
//   exactly the number of hours they missed (start AND end both +N,
//   duration preserved) — e.g. 7-4 shift, arrives at 10 -> becomes 10-7.
export function computeLateStart(emp, arrivalHour) {
  let hoursLate
  if (emp.end <= emp.start) {
    hoursLate = (arrivalHour - emp.start + 24) % 24
    if (hoursLate > 12) hoursLate = 0
  } else {
    hoursLate = arrivalHour - emp.start
  }
  if (hoursLate <= 0) return null // within the grace hour (or not late) — no change
  const newStart = arrivalHour
  const newEnd   = (emp.end + hoursLate) % 24
  return { start: newStart, end: newEnd, hoursShifted: hoursLate }
}
