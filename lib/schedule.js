...
// ── Defaults ───────────────────────────────────────────────────────────
// These lists are the FALLBACK, not the source of truth. The live roster
// and client hours are read from the spreadsheet (see lib/roster.js) so
// they can be changed without touching code or redeploying; what is written
// here is what the platform runs on if that read fails or the tab is empty.
...
// Fallback roster — the live one comes from the Credentials tab.
...
// ── What the platform is actually running on right now ────────────────
// lib/roster.js fills these in from the sheet at the start of every request
// that needs them. Until it does — or if the read fails — the defaults above
// stand, so a spreadsheet problem can never leave the platform with no
// roster and no clients.
let _employees = null
let _timings   = null

export function setScheduleData({ employees, timings } = {}) {
  if (Array.isArray(employees) && employees.length) _employees = employees
  if (timings && Object.keys(timings).length)       _timings   = timings
}
export function employees()    { return _employees || ALL_EMPLOYEES }
export function clientTimings() { return _timings   || CLIENT_TIMINGS }
