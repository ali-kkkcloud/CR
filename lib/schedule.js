...
export function setScheduleData({ employees, timings, employeeHours, hidden, notes } = {}) {
  ...
  if (hidden && typeof hidden === 'object')  _hidden        = hidden
  if (notes  && typeof notes  === 'object')  _notes         = notes
}
...
export function noteFor(client, hour) {
  return _notes[client]?.[hour] || null
}
...
export function specificClientsFor(name, hour) {
  ...
  const kept = c.filter(client => (_timings[client] || []).includes(hour))
