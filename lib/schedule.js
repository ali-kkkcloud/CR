...
export function computeShiftWindow(emp, now) {
  ...
  const base = { duration, extraHour, hoursEarly, isEarly, tooLate: false }

  // ── Early: the whole window moves forward ──
  // Somebody who has logged in wants to work, so the board opens at the hour
  // they arrived and the shift finishes correspondingly earlier.
  if (isEarly) {
  ...
  // ── Late, but the shift is still running: the END DOES NOT MOVE ──
  //
  // A late arrival loses the hours it missed; it does not push the finish
  // later. This used to set the window to "arrival, and a full shift from
  // there", which quietly manufactured a whole new shift out of a late
  // login: an employee rostered 7am–4pm who signed in at 4:06pm was given a
  // 4pm–1am window, handed the entire evening — sixty-five clients at four
  // o'clock, a hundred and eighteen at five — and shown a shift on their own
  // header that they were never rostered for.
  //
  // It also quietly voided the two rules that depend on the end being real:
  // no clients after the shift ends, and auto-close half an hour later. If
  // the operation wants a late arrival to make the time up, OT is the way —
  // deliberate, capped at three hours, once a day.
  if (isScheduledAtHour(emp, arrivalHour)) {
    return {
      ...base, start: arrivalHour, end: emp.end,
      unchanged: arrivalHour === emp.start,
      hoursShifted: ((arrivalHour - emp.start) % 24 + 24) % 24,
    }
  }

  // ── Arrived after the shift was already over ──
  // There is no window to adjust. The attendance row is still recorded — they
  // did sign in, and that is worth knowing — but no override is written, so
  // they are given no clients and the auto-close tidies the row up.
  return {
    ...base, start: emp.start, end: emp.end,
    tooLate: true, unchanged: true, hoursShifted: 0,
  }
}
