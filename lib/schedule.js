...
export function getClientsForEmployeeAtHour(employeeName, hour, redistributedClients = []) {
  ...
  const empIndex   = ALL_EMPLOYEES.findIndex(e => e.name === employeeName)
  ...
  redistributedClients
    ...
    .forEach(r => clients.push({ client: r.client, isRedistributed: true, fromEmployee: r.fromEmployee }))
...
export function redistributeClients(leavingEmployee, currentHour, activeEmployees) {
  ...
  if (!leavingEmp || !activeEmployees.length) return redistribution
