...
export function distributeClientsForHour(hour, scheduledEmployeeNames, vehicleMap = {}, lockedAssignments = {}, reserveOffShiftLocks = false) {
  ...
  const orphanedFixed = []
  employees().forEach(e => {
    if (distribution[e.name] !== undefined) return          // owner is here
...
export function auditHourAssignment(hour, poolNames, vehicleMap = {}, lockedAssignments = {}, reserveOffShiftLocks = false) {
  // Work due this hour is the schedule PLUS any named client whose owner is
  // not in the pool — those are real work somebody has to cover, and a named
  // client that isn't also in Client_Timings would otherwise be invisible here.
  const orphanedFixed = []
  employees().forEach(e => {
    if (poolNames.includes(e.name)) return
    const mine = specificClientsFor(e.name, hour)
    if (mine) orphanedFixed.push(...mine)
  })
  const due = [...new Set([
