...
export function unusableHourRows(rows) {
  ...
  ;(rows || []).slice(1).forEach((r, i) => {
    const name = (r[0] || '').toString().trim()
    const hour = parseInt(r[1], 10)
    const has  = (r[2] || '').toString().trim() || (r[3] || '').toString().trim()
    if (!name && !has) return                       // an empty row is just empty
    if (!name)                out.push({ row: i + 2, reason: 'no employee name' })
    else if (!Number.isInteger(hour) || hour < 0 || hour > 23)
      out.push({ row: i + 2, name, reason: `hour is ${JSON.stringify((r[1] || '').toString())} — must be 0-23` })
    else if (!has)            out.push({ row: i + 2, name, reason: 'no clients and no custom text' })
  })
  return out
}
