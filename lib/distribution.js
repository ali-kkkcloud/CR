...
export function buildHourPool({
  ...
  // Clocked in, but away from the desk long enough that the hour is not
  // theirs — see getAwayOnBreakNames. Left out of the split entirely, because
  // work parked on the board of somebody who isn't there is work nobody is
  // doing, and it is invisible: it does not appear on any colleague's board
  // and nothing flags it until the hour is already missed.
  awayNames = new Set(),
  alwaysInclude = null,
}) {
  ...
  // Anyone away on a long break comes out last — after alwaysInclude, so that
  // somebody whose own page is still polling from behind the break overlay is
  // not handed work either. The moment they hit Resume the break closes and
  // the next refresh puts them back in.
  //
  // If that empties the pool — everybody on shift is away — the hour keeps its
  // owners rather than silently dropping every client off the floor. The same
  // reasoning as the fallback above: better on somebody's board than on
  // nobody's.
  if (awayNames && awayNames.size) {
    const present = poolNames.filter(n => !awayNames.has(n))
    if (present.length > 0) poolNames = present
  }
