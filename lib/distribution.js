...
export function buildHourPool({
  hour, leaveMap, overridesMap, onShiftNames,
  clockedOutNames = new Set(),
  // Who is away from their desk on a break right now.
  //
  // Kept as a parameter because several screens pass it and it is worth
  // SHOWING — but it no longer changes the split. A break is part of a shift,
  // not an absence from it: the employee is at work, they will be back in a
  // few minutes, and the clients scheduled for their hour are theirs.
  //
  // This used to remove them, and it was wrong in exactly the way that matters
  // most here. Their whole hour was handed to colleagues the moment they
  // stepped away, so somebody who took a twelve-minute break came back to an
  // empty board, and the idle auto-break turned that into whole hours reading
  // "0 clients" for people who were sitting at their desks. Work owed to a
  // person on a break is not lost — they still have the hour to do it in, and
  // the Command Center says who is away and for how long so the admin can act
  // if somebody really is gone.
  awayNames = new Set(),
  alwaysInclude = null,
}) {
  ...
  // awayNames is deliberately NOT applied — see the note on the parameter.
