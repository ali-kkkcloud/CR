...
export default function Admin() {
  ...
  function downloadFootageCSV() {
    ...
    a.href = url
    a.download = `Footage_Report_${new Date().toLocaleDateString('en-IN')}.csv`
    a.click()
  }
        ...
        <div style={s.body}>
          ...
          {activeTab === 'overview' && (
            <>
              ...
              <div style={s.sectionHd}>EMPLOYEE STATUS — LIVE</div>
              <div style={s.empGrid}>
                {employees.map(emp => (
                  <div key={emp.name} style={{
                    ...
                    <div style={s.empTop}>
                      ...
                      <div style={{flex:1}}>
                        <div style={{color: emp.statusLabel==='Not Started' ? '#f87171' : '#fff', fontSize:'13px', fontWeight:'600'}}>
                          {emp.name}
                        </div>
                      ...
                      <div style={{...s.statusDot, background: statusColor(emp.statusLabel)}}></div>
                    </div>

                    {emp.statusLabel === 'Active' || emp.statusLabel === 'Ended' ? (
                      <div style={s.empStats}>
                        <div style={s.empStat}>
                          <div style={{color:'#22c55e',fontSize:'16px',fontWeight:'700'}}>{emp.totalUpdates}</div>
                          <div style={{color:'#6b7280',fontSize:'9px'}}>Updates</div>
                        </div>
                        <div style={s.empStat}>
                          <div style={{color: emp.pendingCount>0 ? '#f59e0b' : '#22c55e', fontSize:'16px', fontWeight:'700'}}>{emp.pendingCount}</div>
                          <div style={{color:'#6b7280',fontSize:'9px'}}>Pending</div>
                        </div>
                        <div style={s.empStat}>
                          <div style={{color:'#6b7280',fontSize:'14px',fontWeight:'600'}}>{emp.duration || '—'}</div>
                          <div style={{color:'#6b7280',fontSize:'9px'}}>Duration</div>
                        </div>
                      </div>
                    ) : emp.isWeekOff ? (
                      <div style={{background:'#1a1200',borderRadius:'6px',padding:'8px',textAlign:'center',color:'#fbbf24',fontSize:'11px',marginTop:'8px'}}>
                        Week off — no clients assigned
                      </div>
                    ) : emp.statusLabel === 'Not Started' ? (
                      <div style={{background:'#200808',border:'1px solid #3a1515',borderRadius:'6px',padding:'8px',textAlign:'center',color:'#f87171',fontSize:'11px',marginTop:'8px'}}>
                        Has not started shift
                      </div>
                    ) : null}

                    <div style={{...s.statusBadge, color: statusColor(emp.statusLabel), borderColor: statusColor(emp.statusLabel)+'33', background: statusColor(emp.statusLabel)+'11'}}>
                      {emp.statusLabel}
                    </div>
                  </div>
                ))}
          ...
          {activeTab === 'footage' && (
            <div style={{maxWidth:'800px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
                <div style={s.sectionHd}>ALL FOOTAGE REQUESTS</div>
                <button onClick={downloadFootageCSV} style={s.dlBtn}>⬇ Download CSV</button>
              </div>

              <div style={{display:'flex',gap:'10px',marginBottom:'16px'}}>
                <div style={s.footSumCard}>
                  <span style={{color:'#f59e0b',fontSize:'22px',fontWeight:'700'}}>{footage.pending.length}</span>
                  <span style={{color:'#6b7280',fontSize:'10px'}}>PENDING</span>
                </div>
                <div style={s.footSumCard}>
                  <span style={{color:'#22c55e',fontSize:'22px',fontWeight:'700'}}>{footage.completed.length}</span>
                  <span style={{color:'#6b7280',fontSize:'10px'}}>COMPLETED</span>
                </div>
              </div>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'12px'}}>
                    <div>
                      ...
                      <div style={{color:'#6b7280',fontSize:'11px'}}>
                        ID: {item.issueId} &nbsp;·&nbsp; Raised: {item.raisedAt} &nbsp;·&nbsp; By: {item.raisedBy}
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:'6px',alignItems:'flex-end'}}>
                      ...
                      <button onClick={() => markFootageComplete(item.rowIndex)} style={s.completeBtn}>
                        ✓ Mark Complete
                      </button>
                    </div>
                  </div>
              ...
              {footage.completed.length > 0 && (
                <>
                  <div style={{color:'#374151',fontSize:'10px',letterSpacing:'1px',margin:'20px 0 8px',fontWeight:'600'}}>COMPLETED</div>
                  {footage.completed.map(item => (
                    <div key={item.issueId} style={{...s.footCard, opacity:0.55}}>
                      <div style={{display:'flex',justifyContent:'space-between'}}>
                        <div>
                          ...
                          <div style={{color:'#6b7280',fontSize:'11px'}}>
                            Completed: {item.resolvedAt} · By: {item.raisedBy} · ID: {item.issueId}
                          </div>
                        </div>
                ...
                <table style={s.table}>
                  <thead>
                    <tr style={{background:'#161616'}}>
                      {['From Employee','To Employee','Client','Hour','Reason'].map(h => (
                        <th key={h} style={s.tableHd}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
...
const s = {
  ...
  tabs:           { display:'flex', gap:'4px', borderBottom:'1px solid #222', marginBottom:'20px' },
  tab:            { background:'transparent', border:'none', borderBottom:'2px solid transparent', color:'#6b7280', padding:'10px 16px', fontSize:'13px', cursor:'pointer', marginBottom:'-1px' },
  ...
  footSumCard:    { background:'#111', border:'1px solid #222', borderRadius:'10px', padding:'12px 20px', display:'flex', flexDirection:'column', alignItems:'center', gap:'2px', minWidth:'100px' },
  dlBtn:          { background:'#161616', border:'1px solid #2a2a2a', borderRadius:'8px', color:'#6b7280', fontSize:'12px', padding:'8px 14px', cursor:'pointer' },
  completeBtn:    { background:'#22c55e22', border:'1px solid #22c55e33', borderRadius:'6px', color:'#4ade80', fontSize:'10px', fontWeight:'600', padding:'5px 12px', cursor:'pointer', whiteSpace:'nowrap' },
