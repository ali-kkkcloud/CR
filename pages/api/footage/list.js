...
const COL = { ISSUE_ID:0, CLIENT:2, RAISED_AT:3, LAST_ONLINE:4, SUB_REQUEST:5, DETAILS:6, VEHICLE:9, RAISED_BY:10, RESOLVED:13, RESOLVED_AT:14, LOCATION:15 }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    rows.slice(1).forEach((row, idx) => {
      const subRequest = (row[COL.SUB_REQUEST]||'').toString().trim().toLowerCase()
      if (!subRequest.includes('customer request for video')) return
